...
trait AXI4FFT2RDWithIAandHIStandaloneBlock extends AXI4StreamFFT2RDWithIAandHI[FixedPoint] {
  ...
  val outs: Seq[ModuleValue[AXI4StreamBundle]] = for (o <- 0 until nOut) yield {
    ...
    out := AXI4StreamToBundleBridge(
      AXI4StreamSlavePortParameters(AXI4StreamSlaveParameters())
    ) := AXI4StreamBuffer() := headerInserter.streamNode
  ...
  val ins: Seq[ModuleValue[AXI4StreamBundle]] = for (i <- 0 until nIn) yield {
    ...
    inputAdapter.streamNode := BundleBridgeToAXI4Stream(
...
class AXI4StreamFFT2RDWithIAandHI[T <: Data: Real: BinaryRepresentation](
  val params:    FFT2RDParams[T],
  mode1024:      Boolean = true,
  hIAddress:     AddressSet,
  val beatBytes: Int)
    extends LazyModule()(Parameters.empty) {

  val inputAdapter = LazyModule(new AXI4StreamAdapter1to4(beatBytes = beatBytes))
  val fft2Control = LazyModule(
    new AXI4StreamFFT2RDControlBlock(params.fft2ControlParams, params.fft2ControlAddress, beatBytes = beatBytes)
  )
  val rangeFFT = LazyModule(
    new AXI4MultipleFFTsBlock(
      params.rangeFFTParams,
      params.rangeFFTAddress,
      _beatBytes = beatBytes,
      configInterface = false
    )
  )
  val dopplerFFT = LazyModule(
    new AXI4MultipleFFTsBlock(
      params.dopplerFFTParams,
      params.dopplerFFTAddress,
      _beatBytes = beatBytes,
      configInterface = false
    )
  )
  val zeroPadderRange =
    if (params.zeroPadderRangeParams != None)
      Some(LazyModule(new AXI4MultipleZeroPadders(params.zeroPadderRangeParams.get, params.zeroPadderRangeAddress.get)))
    else None
  val zeroPadderDoppler =
    if (params.zeroPadderDopplerParams != None)
      Some(
        LazyModule(new AXI4MultipleZeroPadders(params.zeroPadderDopplerParams.get, params.zeroPadderDopplerAddress.get))
      )
    else None
  val windowingRange =
      ...
      Some(
        LazyModule(
          new WindowingBlockMultipleInOuts(
            params.winRangeAddress.get,
            params.winRangeRAMAddress,
            params.winRangeParams.get,
            beatBytes
          )
        )
      )
    else None
  val windowingDoppler =
    if (params.winDopplerParams != None)
      Some(
        LazyModule(
          new WindowingBlockMultipleInOuts(
            params.winDopplerAddress.get,
            params.winDopplerRAMAddress,
            params.winDopplerParams.get,
            beatBytes
          )
        )
      )
...
object FFT2RDWithIAandHIDspBlockAXI4 extends App {
  ...
  val paramsFFT2RD: FFT2RDParams[FixedPoint] = FFT2RDParams(
    ...
    winDopplerRAMAddress =
      None, //Some(AddressSet(0x10000, 0xFFFF)), // constWindow needs to be set to true if it is None
  ...
  val fft2Module = LazyModule(
    new AXI4StreamFFT2RDWithIAandHI(paramsFFT2RD, mode1024, hiAddress, beatBytes = 4)
