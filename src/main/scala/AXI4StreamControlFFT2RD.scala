...
abstract class FFT2RDControlBlock[D, U, E, O, B <: Data](params: FFT2RDControlParams, beatBytes: Int)
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val frameCounter =
      RegInit(0.U(log2Ceil(frameSize + 1).W)) // frame size should be renamed, the name is not intuitive at all

    var fields = Seq(
      // settable registers
      RegField(
        log2Ceil(rangeFFTSize + 1),
        numSamples,
        RegFieldDesc(name = "numSamples", desc = "Configure number of samples inside chirp")
      ),
      RegField(
        log2Ceil(dopplerFFTSize + 1),
        numChirps,
        RegFieldDesc(name = "numChirps", desc = "Configure number of chirps")
      ), // important for writing data into memory, when to stop
      RegField(
        log2Ceil(dopplerFFTSize + 1),
        dopplerSize,
        RegFieldDesc(name = "dopplerFFTSize", desc = "Configure doppler FFT size")
      ), // important for reading
      RegField(
        log2Ceil(rangeFFTSize + 1),
        activeRangeSamples, // should be used instead of numSamples on Doppler side
        RegFieldDesc(
          name = "activeRangeSamples",
          desc = "Configure number of active range samples that should be used later for Doppler FFT"
        )
      ),
      RegField(
        log2Ceil(params.numTxs + 1),
        numTxs,
        RegFieldDesc(name = "numTxs", desc = "Configure number of transmitters")
      ),
      RegField(
        log2Ceil(params.numRxs + 1),
        numRxs,
    ...
    val readDir =
      if (params.outputNodes == 1 && params.incRegXYZorXZY.getOrElse(false)) Some(RegInit(false.B)) else None

    if (params.outputNodes == 1 && params.incRegXYZorXZY.getOrElse(false)) {
      fields =
        fields :+ RegField(1, readDir.get, RegFieldDesc(name = "readDirection", desc = "Defines reading out direction"))
    }
    ...
    val syncReadMems1 =
      if (params.pingPong) Some(Seq.fill(numMems)(SyncReadMem(rangeFFTSize * dopplerFFTSize, protoData))) else None
    ...
    val inFire =
      ins.zip(activeRxs).map { case (in, active) => Mux(active, in.valid & in.ready, true.B) }.reduce((a, b) => a & b)
    ...
    if (params.outputNodes > 1) {
      when(
        ((pingPongRead.getOrElse(startRead) =/= pingPongWrite.getOrElse(false.B)) || forbidWrite.getOrElse(
          false.B
        )) && out.ready === true.B
      ) {
...
//     }
    ...
    val readAddress1 =
      if (params.pingPong) Some(Wire(Vec(params.numRxs * params.numTxs, readAddress.cloneType))) else None
    if (params.pingPong) {
      readAddress1.get.map { case (readAddr) => readAddr := readAddress }
    }

    if (params.outputNodes == 1) {
      val cases = if (params.pingPong == false) (0 until params.numTxs * params.numRxs).map(c => c.U).zipWithIndex.map {
        case (txIndex, indexTx) => //maybe some renaming will be applied here
          (txIndex === RegNext(RegNext(frameCounter))) -> syncReadMems0((indexTx))(readAddress0((indexTx)))
      }
      else
        (0 until params.numTxs * params.numRxs).map(c => c.U).zipWithIndex.map {
          case (txIndex, indexTx) =>
            ...
            (txIndex === RegNext(RegNext(frameCounter))) -> Mux(
              RegNext(RegNext(pingPongRead.get, false.B)),
              syncReadMems1.get((indexTx))(readAddress1.get((indexTx))),
              syncReadMems0((indexTx))(readAddress0((indexTx)))
            )
        }
      ...
      for (i <- 0 until params.numRxs) {
        val cases = if (params.pingPong == false) numTxIndices.zipWithIndex.map {
          case (txIndex, indexTx) =>
            (txIndex === RegNext(RegNext(frameCounter))) -> syncReadMems0((indexTx * params.numRxs + i))(
              readAddress0((indexTx * params.numRxs + i))
            )
        }
      ...
      for (i <- 0 until params.outputNodes) {
        if (params.pingPong == false) { // no need to change anything this should work!
          ...
          outsData(i) := Mux(
            RegNext(RegNext(pingPongRead.get, false.B)),
            syncReadMems1.get(i)(readAddress1.get(i)),
            syncReadMems0(i)(readAddress0(i))
          )
        }
      }
    }
    ...
    val validOut =
      if (params.pingPong) RegNext(RegNext(((pingPongRead.get =/= pingPongWrite.get) || forbidWrite.get), false.B))
      else RegNext(RegNext(startRead))
    outs.zipWithIndex.map {
      case (out, i) =>
        val outQueue =
          Module(
            new Queue(chiselTypeOf(out.bits.data), entries = 3, pipe = true, flow = false)
          ) // pipe = true, flow = true))
        ...
        val outQueueLast =
          Module(
            new Queue(chiselTypeOf(out.bits.last), entries = 3, pipe = true, flow = false)
          ) //pipe = true, flow = true))
...
class AXI4StreamFFT2RDControlBlock(
  val params:    FFT2RDControlParams,
  address:       AddressSet,
  val beatBytes: Int = 4
)(
  implicit p: Parameters)
    extends FFT2RDControlBlock[
