...
trait AXI4FFT2RDStandaloneBlock extends AXI4StreamFFT2RDBlock[FixedPoint] {
  ...
  val ins: Seq[ModuleValue[AXI4StreamBundle]] = for (i <- 0 until nIn) yield {
    ...
    if (params.zeroPadderRangeParams != None) {
      zeroPadderRange.get.streamNode := BundleBridgeToAXI4Stream(
        AXI4StreamMasterPortParameters(AXI4StreamMasterParameters(n = beatBytes))
      ) := in
    } else {
      if (params.winRangeParams != None) {
        windowingRange.get.streamNode := BundleBridgeToAXI4Stream(
          AXI4StreamMasterPortParameters(AXI4StreamMasterParameters(n = beatBytes))
        ) := in
      } else {
        rangeFFT.streamNode := BundleBridgeToAXI4Stream(
          AXI4StreamMasterPortParameters(AXI4StreamMasterParameters(n = beatBytes))
        ) := in
      }
    }
...
class AXI4StreamFFT2RDBlock[T <: Data: Real: BinaryRepresentation](val params: FFT2RDParams[T], val beatBytes: Int)
...
object FFT2RDDspBlockAXI4 extends App {
  ...
  val paramsFFT2RD: FFT2RDParams[FixedPoint] = FFT2RDParams(
    ...
    zeroPadderRangeParams = None,
    /*Some(ZeroPadderParams(
