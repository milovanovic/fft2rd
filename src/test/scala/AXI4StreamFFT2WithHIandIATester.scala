...
class AXI4StreamFFT2RDWithIAandHIBlockSpec extends AnyFlatSpec with Matchers {
  ...
  for (i <-0 until numOfIterations) {
    for (dir <- Seq(false)) {//, false)) {
      ...
      val inFileNameComplex : String = f"./generators/dsp-blocks/fft2rd/python/gen_data_dir/complexDataInHexWithInputAdapter$readDir.txt"
      val outFileNameComplex: String = f"./generators/dsp-blocks/fft2rd/python/gen_data_dir/complexDataOutHexWithHeaderInserter$readDir.txt"
      ...
      it should f"test Range-Doppler 2D-FFT module, results are compared with Python model of 2D-FFT design - iteration $i, with read direction equal to $readDir," in {
        chisel3.iotesters.Driver.execute(Array("-fiwv",
              "--backend-name", "verilator"), () => testModule.module) {
