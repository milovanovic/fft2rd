package utils // rename package name to dsputils
...
abstract class CustomMux[D, U, E, O, B <: Data](beatBytes: Int) extends LazyModule()(Parameters.empty) with DspBlock[D, U, E, O, B] with HasCSR {
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val fields = Seq(RegField(log2Up(ins.length + 1), selSignal, RegFieldDesc(name = "selSignal", desc = "Register used to select appropriate input")))
    regmap(fields.zipWithIndex.map({ case (f, i) => i * beatBytes -> Seq(f)}): _*)
  }
}

class AXI4StreamCustomMux(address: AddressSet, val beatBytes: Int = 4)(implicit p: Parameters) extends CustomMux[AXI4MasterPortParameters, AXI4SlavePortParameters, AXI4EdgeParameters, AXI4EdgeParameters, AXI4Bundle](beatBytes) with AXI4DspBlock with AXI4HasCSR {
...
object CustomMuxApp extends App
{
  ...
  (new ChiselStage).execute(Array("--target-dir", "verilog/AXI4StreamCustomMux"), Seq(ChiselGeneratorAnnotation(() => lazyDut.module)))
}
