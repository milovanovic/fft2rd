...
abstract class DataStoringModule [D, U, E, O, B <: Data](beatBytes: Int, totalData: Int, triggerIn: Boolean) extends LazyModule()(Parameters.empty) with DspBlock[D, U, E, O, B] with HasCSR {
   ...
   //   when (out.fire()) {
     ...
     /* out.bits.data := readData
      ...
      outQueue.io.enq.valid := RegNext(out.ready && triggerReg, false.B) && state === sLoad //RegNext(state === sLoad && out.ready && triggerReg, false.B) // RegNext(state === sLoad && out.ready, false.B)
      ...
      val fields = Seq(RegField(log2Up(totalData+1), expectedData,   RegFieldDesc(name = "total_data", desc = "Define number of data that should be considered in the process of reading and writing")))
...
class AXI4StreamDataStoringModule(address: AddressSet, beatBytes: Int = 4, totalData: Int, triggerIn: Boolean = false)(implicit p: Parameters) extends DataStoringModule[AXI4MasterPortParameters, AXI4SlavePortParameters, AXI4EdgeParameters, AXI4EdgeParameters, AXI4Bundle](beatBytes, totalData, triggerIn) with AXI4DspBlock with AXI4HasCSR {
...
object AXI4StreamDataStoringApp extends App
{
  ...
  val lazyDut = LazyModule(new AXI4StreamDataStoringModule(AddressSet(0x00, 0xF), beatBytes = 4, totalData, true) with AXI4StreamDataStoringPins{override def numBytes = 4})
